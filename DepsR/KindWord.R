...
for (kind in kinds) {
    project.messages <- paste(project.name, paste(allorone, paste(kind, "message_b.csv", sep = ""), sep = ""), sep = "/")
...
write.csv(kind_message, paste(project.name, paste(allorone, "message_b.csv", sep = ""), sep = "/")
